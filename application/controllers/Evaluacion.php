...
class Evaluacion extends CI_Controller {
    ...
    public function reporte_excel($convocatoria_id, $inscripcion_id) {
       $fecha_elegida = $this->input->get('fecha_elegida');
    //    if ($fecha_elegida =='0') {
        if (true) {
           //reporte total aqui
...
//        $hoja->setCellValue('H2', 'HORAS EFECTIVOS')->getStyle('H2')->getFont()->setSize(15)->setBold(true);
       ...
       foreach ($datos as $fila) {
           ...
           $hoja->setCellValue('A' . $cont, $fila['especialidad_completa'], PHPExcel_Cell_DataType::TYPE_STRING);
           $hoja->getStyle('B' . $cont)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);

           $hoja->setCellValue('B' . $cont, utf8_encode(" ".$fila['dni']) );
           ...
           $hoja->setCellValue('I' . $cont, $fila['suma_final']);
           ...
           $hoja->setCellValue('K' . $cont, $fila['num_exp'], PHPExcel_Cell_DataType::TYPE_STRING);
           $hoja->getStyle('L' . $cont)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
           $hoja->setCellValue('L' . $cont, $fila['nombre_especialista'], PHPExcel_Cell_DataType::TYPE_STRING);

           $cont++;
       }

       $filename = 'REPORTE_PUBLICACION.xls'; //save our workbook as this file name
...
//force user to download the Excel file without writing it to server's HD
       ...
       if ($fecha_elegida !='0') {
           $datos = []; //$this->evaluacion_expediente->reporte_pdf($fecha_elegida);
       /* A partir de ahora cualquier salida al navegador se guardarÃ¡ en un buffer */
       /* Obtenemos el listado de locales disponibles en el sistema */
       file_put_contents('log.txt', shell_exec('locale -a'), FILE_APPEND);
       set_time_limit(0);
