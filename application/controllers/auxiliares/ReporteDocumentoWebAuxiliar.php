...
class ReporteDocumentoWebAuxiliar extends CI_Controller
{
    ...
    public function reporte_personal_pdf()
    {
        ...
        $this->pdf->SetAligns(array('C', 'C', 'C'));
        $this->pdf->Row(array(
            utf8_decode("b.1 Programas de Formación Auxiliar, Actualización, Especialización o Segunda Especialización, afín al área curricular o campo de conocimiento que postula:
        ...
        $this->pdf->SetAligns(array('C', 'C', 'C'));
        $this->pdf->Row(array(
            utf8_decode("b.2 Cursos o Módulos de Formación Auxiliar, afín al área curricular o campo de conocimiento que postula:
            -Realizado en los últimos cinco (05) años.
            -Presenciales, virtuales o semipresenciales.
        ...
        $this->pdf->SetAligns(array('C', 'C', 'C'));
        $this->pdf->Row(array(
            utf8_decode("c.1 Experiencia Laboral Auxiliar, en la modalidad educativa o el nivel educativo o ciclo al que postula, durante los meses de marzo a diciembre, teniendo en cuenta:
        ...
        // $this->pdf->Ln(70);
        // $this->pdf->Cell(90, 0, utf8_decode('Comision de contratacion Auxiliar 2018 - UGEL 04 '), '', 0, 'L', 0);
        $this->pdf->Ln(18);
        $this->pdf->SetFont('Arial', '', 9);
        $this->pdf->Cell(90, 0, utf8_decode('Comision de contratacion Auxiliar ' . date('Y') . ' - ' . date("d/m/y" . " ") . 'UGEL N° 05 - SJL'), '', 0, 'L', 0);
        $this->pdf->Ln(5);
    ...
    public function reporte_personal_pdf_do()
    {
        ...
        //un arreglo con alineacion de cada celda
        $this->pdf->SetAligns(array('C', 'C', 'C'));
        $this->pdf->Row(array(utf8_decode("b.1 Programas de Formación Auxiliar, Actualización, Especialización o Segunda Especialización, afín a la modalidad y nivel al que postula: - Realizado en los últimos cinco (05) años. - Presenciales, virtuales o semipresenciales - Duración mínima de 126 horas cronológicas o 7 créditos. - Dos (02) puntos por cada uno de ellos."), utf8_decode($datos[0]['b1'])));

        $this->pdf->Cell(6, 5, '', 0, 0, 'C', false);
        $this->pdf->SetWidths(array(165, 20, 25));
        //un arreglo con alineacion de cada celda
        $this->pdf->SetAligns(array('C', 'C', 'C'));
        $this->pdf->Row(array(utf8_decode("b.2 Cursos o Módulos de Formación Auxiliar, afín a la modalidad y nivel al que postula: - Realizado en los últimos cinco (05) años. - Presenciales, virtuales o semipresenciales. - Duración mínima de 36 horas cronológicas. - Dos (02) puntos por cada uno de ellos."), utf8_decode($datos[0]['b2'])));
        ...
        //un arreglo con alineacion de cada celda
        $this->pdf->SetAligns(array('C', 'C', 'C'));
        $this->pdf->Row(array(utf8_decode("c.1 Experiencia Laboral Auxiliar, en el nivel, ciclo y modalidad educativa al que postula, durante los meses de marzo a diciembre, teniendo en cuenta, durante los meses de marzo a diciembre, teniendo en cuenta: -Corresponde 0.20 puntos por cada mes acreditado de labor en IE ubicada en zona urbana. -Corresponde 0.30 puntos por cada mes acreditado de labor en IE ubicada en zona de frontera. -Corresponde 0.30 puntos por cada mes acreditado de labor en IE ubicada en zona rural. -Corresponde 0.40 puntos por cada mes acreditado de labor en IE ubicada en zona VRAEM. -Máximo a considerar 50 meses --Máximo a considerar 10 meses por cada año lectivo. -Un mes equivale a 30 días -No corresponde puntaje por periodos menores a 30 días, ni es acumulable los días para completar un mes (30 días)."), utf8_decode($datos[0]['c1'])));
    ...
    public function reporte_personal_pdf_ce()
    {
        ...
        $this->pdf->Cell(6, 5, '', 0, 0, 'C', false);
        $this->pdf->Cell(165, 5, utf8_decode('c.2 Experiencia Auxiliar en Educación Superior o Técnico – productiva'), 1, 0, 'C');
    ...
    public function reporte_personalizado()
    {
        ...
        $this->pdf->Cell(50, 5, 'Auxiliar', 1, 0, 'C', true);
    ...
    public function adjudicacion_plaza()
    {
        ...
        $this->pdf->Ln(6);
        //******* SUBTITULO DEL Auxiliar********//
        $this->pdf->Ln(8);
        $this->pdf->Cell(5, 7, '', '', 0, 'C', 0);
        $this->pdf->SetFont('Arial', 'BU', 9);
        $this->pdf->Cell(40, 7, utf8_decode('DATOS DEL Auxiliar'), '', 0, 'L', 0);
        $this->pdf->SetFont('Arial', '', 8);
        //******* NOMBRES ********//
        $this->pdf->Ln(7);
        $this->pdf->Cell(5, 7, '', '', 0, 'C', 0);
 ...
 */
        ...
        $this->pdf->Cell(35, 7, 'Firma del Auxiliar', '', 0, 'LU', 0);
        $this->pdf->Ln(4);
    ...
    public function reserva()
    {
        ...
        // $this->Cell(20, 4,utf8_decode('De conformida con el resultado obtenido en el Proceso para Contratación de Auxiliars, regulado por la Norma Técnica aprobada con R.M. Nº 023-2015-MINEDU, se adjunta el cargo vacante a:'),'',0, 'L', 0);
        $this->pdf->Cell(5, 7, '', '', 0, 'C', 0);
        $this->pdf->MultiCell(180, 7, utf8_decode('En SJL, a los ' . date_format(date_create($datos[0]['fechaReg']), "d") . ' días del mes de ' . $this->meses[date_format(date_create($datos[0]['fechaReg']), "m") - 1] . ' del año ' . date_format(date_create($datos[0]['fechaReg']), "Y") . ', siendo las ' . date_format(date_create($datos[0]['fechaReg']), "g:i A") . ' horas, los miembros del Comité de Contratación Auxiliar de la ' . $datos[0]['ugel'] . ', y el Auxiliar ' . $datos[0]['nombres'] . ' de la especialidad de ' . utf8_decode($datos[0]['especialidad']) . ' en uso de su derecho que le confiere del ' . $datos[0]['descrip_doc'] . 'D.S. Nº 001-2017' . $datos[0]['numero'] . '-MINEDU, que aprueba la Norma que regula el procedimiento, requisitos y condiciones para las contrataciones en el marco del contrato del servicio Auxiliar a que hace referencia la Ley Nº 30328. Ley que establece medidas en materia educativa y dicta otras disposiciones, cuyo texto en calidad de Anexo, forma parte integrante del presente ' . $datos[0]['descrip_doc'] . ', y acordamos elaborar el acta en los siguientes términos:'), 0);
        $this->pdf->Ln(3);
        ...
        $this->pdf->SetFont('Arial', '', 10);
        $this->pdf->Cell(5, 7, '', '', 0, 'C', 0);
        $this->pdf->MultiCell(180, 7, utf8_decode('Que, el Auxiliar voluntariamente elige la plaza vacante de la institución educativa ' . $datos[0]['ie'] . ', código de plaza Nº ' . $datos[0]['codigo_plaza'] . ', en uso de sus derecho amparada por la norma acotada. Sin lugar a reclamo alguno posterior en caso que la plaza sea ocupada por reubicación de excedente, reasignación por salud u otro.'), 0);
        $this->pdf->Ln(15);
    ...
    public function contrato($id)
    {
        ...
        $this->pdf->SetFont('', '');
        $this->pdf->Cell(5, 5, '', '', 0, 'R', 0);
        $this->pdf->MultiCell(180, 5, utf8_decode('En atención a las necesidades de contar con los servicios de un Profesional Auxiliar, el Comité de Contratación de la DRE / UGEL adjudicó la plaza orgánica/eventual/temporal/horas al señor(a) ' . $postulante->nombre . ' para desempeñar funciones Auxiliars.'), 0);
        ...
        $this->pdf->SetFont('', '');
        $this->pdf->Cell(5, 5, '', '', 0, 'R', 0);
        $this->pdf->MultiCell(180, 5, utf8_decode('El PROFESOR en virtud al presente contrato de servicio Auxiliar percibe la remuneración mensual fijada por Decreto Supremo. Adicionalmente en caso corresponda, percibe los derechos y beneficios de conformidad a la normativa específica que lo regula.'), 0);
        ...
        $this->pdf->MultiCell(170, 5, utf8_decode('No cumplir con los requisitos para la contratación Auxiliar establecidos en la presente norma.'), 0);
        ...
        $this->pdf->SetFont('', '');
        $this->pdf->Cell(5, 5, '', '', 0, 'R', 0);
        $this->pdf->MultiCell(180, 5, utf8_decode('El presente contrato de servicio Auxiliar es vigente a partir del plazo establecido en la cláusula tercera.'), 0);
        ...
        $this->pdf->SetFont('', '');
        $this->pdf->Cell(5, 5, '', '', 0, 'R', 0);
        $this->pdf->MultiCell(180, 5, utf8_decode('El presente contrato de servicio Auxiliar se aprueba mediante la resolución administrativa correspondiente.'), 0);
...
//        $this->pdf->MultiCell(170, 5, utf8_decode('No haber sido sancionado administrativamente con destitución, cese temporal, suspensión, amonestación o separación del servicio en el periodo comprendido en los últimos cinco años, a la fecha inclusive.'), 0);
        ...
        $this->pdf->Ln(1);
        $this->pdf->Cell(10, 5, '-', '', 0, 'R', 0);
        $this->pdf->MultiCell(170, 5, utf8_decode('No encontrarme impedido de prestar labor Auxiliar efectiva, conforme al marco normativo vigente.'), 0);
        $this->pdf->Ln(1);
        $this->pdf->Cell(10, 5, '-', '', 0, 'R', 0);
        ...
        $this->pdf->Ln(1);
        $this->pdf->Cell(10, 5, '-', '', 0, 'R', 0);
        $this->pdf->MultiCell(170, 5, utf8_decode('No haber presentado renuncia a contrato Auxiliar en el ámbito de la región a la que postuló.'), 0);
        $this->pdf->Ln(1);
        $this->pdf->Cell(10, 5, '-', '', 0, 'R', 0);
...
//        $this->pdf->MultiCell(180, 5, utf8_decode('Nota: Si el postulante oculta la información y/o consigna información falsa será excluido del proceso de selección de personal. En caso de haberse producido la contratación, se deberá proceder con la conclusión o anulación del contrato por comisión de falta grave, con arreglo a las normas vigentes, sin perjucio de la responsabilidad penal que hubiera incurrido (Art. 4° DS 017-96-PCM )'), 0);
    ...
    public function anexo6B()
    {
        ...
        //        $datos_frmAuxiliar = $this->documento->obtener_datos_frmAuxiliar($numDoc);
    ...
    public function anexo7()
    {
        ...
        $this->pdf->MultiCell(170, 5, utf8_decode('1. Que, tengo conocimiento que ningún funcinario o servidor público puede desempeñar más de un empleo o cargo público remunerado, *con excepción de uno o más por función Auxiliar.*'), 0);
        $this->pdf->Ln(2);
        $this->pdf->Cell(10, 5, '', '', 0, '', 0);
        ...
        $this->pdf->SetFont('Arial', '', 8);
        $this->pdf->Cell(35, 7, 'Frima del Auxiliar ', '', 0, 'LU', 0);
    ...
    public function resolucion($id)
    {
        ...
        $this->pdf->SetFont('Arial', '', 10);
        $this->pdf->Cell(5, 7, '', '', 0, 'C', 0);
        $this->pdf->MultiCell(180, 5, utf8_decode('                 Que, el artículo 76° de la Ley N° 29944, Ley de Reforma Magisterial dispone que las plazas vacantes existentes en las instituciones educativas públicas no cubiertas por nombramiento son atendidas vía concurso público de contratación Auxiliar'), 0);
        $this->pdf->SetFont('Arial', '', 10);
        $this->pdf->Ln(4);

        $this->pdf->SetFont('Arial', '', 10);
        $this->pdf->Cell(5, 7, '', '', 0, 'C', 0);
        $this->pdf->MultiCell(180, 5, utf8_decode('                 Que, el artículo 1° de la Ley Nº 30328, Ley que establece medidas en materia educativa y dicta otras disposiciones, señala que el Contrato de Servicio Auxiliar regulado en la Ley de Reforma Magisterial tiene por finalidad permitir la contratación temporal del profesorado en instituciones educativas públicas de educación básica y técnico productiva; es de plazo determinado y procede en el caso que exista plaza vacante en las instituciones educativas;'), 0);
        $this->pdf->SetFont('Arial', '', 10);
        $this->pdf->Ln(4);

        $this->pdf->SetFont('Arial', '', 10);
        $this->pdf->Cell(5, 7, '', '', 0, 'C', 0);
        $this->pdf->MultiCell(180, 5, utf8_decode('                 Que, por Decreto Supremo Nº 020-2023-MINEDU, se aprueba la Norma que regula el procedimiento para las contrataciones de profesores y su renovación en el marco del contrato de servicio Auxiliar en educación básica y técnico productiva, a que hace referencia la ley N° 30328, ley que establece medidas en materia educativa y dicta otras disposiciones, con el objetivo de establecer disposiciones en relación al procedimiento, requisitos y condiciones para la contratación de profesores y la renovación de su contrato, en los programas educativos y en las IIEE públicas de Educación Básica y Técnico Productiva'), 0);
        $this->pdf->SetFont('Arial', '', 10);
        $this->pdf->Ln(4);

        $this->pdf->SetFont('Arial', '', 10);
        $this->pdf->Cell(5, 7, '', '', 0, 'C', 0);
        $this->pdf->MultiCell(180, 5, utf8_decode('                 Estando a lo actuado por el comité de contratación Auxiliar / Jefe de Recursos Humanos, con el visto bueno de las dependencias correspondientes de la UGEL, el contrato suscrito entre el Auxiliar adjudicado y el titular de la entidad, y;'), 0);
        $this->pdf->SetFont('Arial', '', 10);
        $this->pdf->Ln(4);
        ...
        $this->pdf->SetFont('Arial', '', 10);
        $this->pdf->Cell(5, 7, '', '', 0, 'C', 0);
        $this->pdf->MultiCell(180, 5, utf8_decode('                 ARTÍCULO 1º.- APROBAR EL CONTRATO, por servicios personales según el anexo que forma parte de la presente, suscrito por la Unidad Ejecutora y el personal Auxiliar que a continuación se indica:'), 0);
        $this->pdf->SetFont('Arial', '', 10);
        ...
        $this->pdf->SetFont('Arial', '', 10);
        $this->pdf->Cell(5, 7, '', '', 0, 'C', 0);
        $this->pdf->MultiCell(180, 5, utf8_decode('               ARTICULO 2°.- ESTABLECER, conforme al Anexo 1 del Decreto Supremo N° 020-2023-MINEDU, que contiene el documento "Contrato de Servicio Auxiliar", es causal de resolución del contrato cualquiera de los motivos señalados en la Cláusula Sexta.'), 0);
        $this->pdf->SetFont('Arial', '', 10);
        $this->pdf->Ln(4);
...
//        $this->pdf->setCodVerificacion($datos [0] ['detalleID']);
    ...
    public function postulacion($uid)
    {
        ...
        $response = $this->postulaciones_auxiliar_model->detail_ficha_postulacion(['uid' => $uid]);
        ...
        $this->pdf->Cell(0, 10, utf8_decode('FICHA DE POSTULACIÓN PARA EL PROCESO DE CONTRATO AUXILIAR'), 0, 1, 'C', true);
        $this->pdf->Ln(5);
        ...
        $this->pdf->SetTextColor(50, 50, 100);  // Texto en azul oscuro
        $this->pdf->SetFont('Arial', 'I', 8);
        $this->pdf->Cell(0, 10, utf8_decode('Ficha consolidada para el Proceso de Contrato Auxiliar - 2024'), 0, 0, 'L');
    ...
    public function reclamo($uid)
    {
        ...
        $this->pdf->SetFillColor(220, 220, 220);  // Fondo gris claro
        $this->pdf->SetXY(10, 45);
        $this->pdf->Cell(0, 10, utf8_decode('FICHA DE RECLAMO PARA EL PROCESO DE CONTRATO AUXILIAR'), 0, 1, 'C', true);
        $this->pdf->Ln(5);
