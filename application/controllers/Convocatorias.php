...
class Convocatorias extends CI_Controller {
    ...
    public function listar(){   // TIENE SOLO 2 SEGMENTOS
        ...
        $this->layout->js(array(base_url()."public/js/myscript/convocatorias/listar.js?t=".date("mdYHis")));
    ...
    public function VListarConvocatorias(){
        ...
        $datos  = $this->convocatorias_model->listarConvocatoriasTodas($idPer, $idPro);     
        $this->layout->setLayout("template_ajax");
    ...
    public function VSelectGrupoInscripcion(){ 
        ...
        $grupos = $this->configuracion_model->listarGruposInscripcion($idPer, $idPro);      
        $this->layout->setLayout("template_ajax");
        $this->layout->view('listar/VSelectGrupoInscripcion', compact('grupos'));
    }
    
    public function VTablaGrupoInscripcion(){
        $grupoArr   = $this->input->post("grupoArr",true);
        if(!empty($grupoArr)){
            $datos = $this->convocatorias_model->listarGruposInscripcionxIds($grupoArr);
        }else{
            $datos = [];
        }
        $this->layout->setLayout("template_ajax");
        $this->layout->view('listar/VTablaGrupoInscripcion', compact('datos'));
    }
    ...
    public function cargarexpedientes($cadena = null){   // TIENE SOLO 3 SEGMENTOS
        ...
        if($idCon != '0' && $idGin == '0'){
            $datos   = $this->convocatorias_model->listarGruposInscripcionxConvocatoria($idCon);
        ...
        if($idCon != '0' && $idGin != '0'){
            $dato   = $this->convocatorias_model->listarGrupoInscripcionxConvocatoriaYEspecialidad($idCon, $idGin);
        ...
        switch ($tipoEval) {
            case '1':
                ...
                if($estado==1){ // sin evaluacion
                    $datos    = $this->convocatorias_model->listarCuadroPunxIdGrupoSinEvaluacion($idGin);
                }else{ // todos
                    $datos    = $this->convocatorias_model->listarCuadroPunxIdGrupoTodos($idGin);
                }                
            ...
            case '2':
                if($estado==1){ // sin evaluacion
                    $datos    = $this->convocatorias_model->listarCuadroExpxIdGrupoSinEvaluacion($idGin);
                }else{ // todos
                    $datos    = $this->convocatorias_model->listarCuadroExpxIdGrupoTodos($idGin);
                }
    ...
    public function DTExpedientes(){
        ...
        $buscarDocentes = $this->convocatorias_model->buscarDocentesPun($idGin);
        ...
        foreach ($datos as $dato) { 
             ...
             */
            $buscarAsignacion = $this->convocatorias_model->buscarAsignacionExpediente($dato->tra_anio, $dato->tra_numero);
    ...
    public function CAsisgnarExpedientes(){
        ...
        for ($i=0; $i < count($expedientes) ; $i++) { 
            ...
            if (!empty($archivosMPV)) {   // si existe en mesa de partes 
                ...
                $expId	= $this->convocatorias_model->insertExpediente($arrInsert);
                ...
                $insertId	= $this->convocatorias_model->insertArchivosDetalleBatch($arrInsert);  
                ...
                if($key >= 0){ 
                    ...
                    $insertId	= $this->convocatorias_model->insertAsigancionExpedientePun($arrInsert);  
                    if ($insertId<=0){ // NO SE HA REGISTRADO EL ARCHIVO.
                        $mensaje["error"]="Error al registar expedientes.";                         
                        $mensaje["estado"]=false; 
                        echo json_encode($mensaje); 
    ...
    public function CEnviarEvaluar(){        
        ...
        $update=$this->convocatorias_model->updateBatchCuadroPun($ar_update);
        if($update > 0){
    ...
    public function CQuitarAsisgnacionExpedientes(){        
        ...
        $update=$this->convocatorias_model->updateBatchAsignacionExpedientePunxIdPun($ar_update);
        if($update > 0){
    ...
    public function DTExpedientesExp(){
        ...
        $buscarDocentes = $this->convocatorias_model->buscarDocentesExp($idGin);
        $datos  = $this->mesadepartes_model->buscarExpedientesProcesados($F_desde, $F_hasta, $TramiteId);
        $arreglo=[];
        $i=0;
        $flag="";
    ...
    public function CAsisgnarExpedientesExp(){
        ...
        for ($i=0; $i < count($expedientes) ; $i++) { 
            ...
            if($key < 0){ // no existe
                ...
                $insertId	= $this->convocatorias_model->insertCuadroPunExp($arrInsert);  
                if ($insertId<=0){ // NO SE HA REGISTRADO EL ARCHIVO.
                    $mensaje["error"]="Error al registar expedientes.";                         
                    $mensaje["estado"]=false; 
                    echo json_encode($mensaje); 
