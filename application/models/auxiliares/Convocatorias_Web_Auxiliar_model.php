...
class Convocatorias_Web_Auxiliar_model extends CI_Model
{
  ...
  public function index($idPer, $idPro)
  {
    $sql = $this->db
      ...
      ->join("auxiliar_tipo_convocatoria tcon", "tcon.tipo_id = con.con_tipo", "inner")
  ...
  public function showReclamo($request)
  {
    $response = $this->tools->responseDefault();
    try {
      ...
      if (!$convocatoria) {
        ...
        throw new Exception("No se encontro la etapa de reclamo de la convocatoria");
      }

      $now_unix = strtotime($this->tools->getDateHour());

      $con_fechainicio_unix = strtotime($convocatoria->con_fechainicio_reclamo . ' ' . substr($convocatoria->con_horainicio_reclamo, 0, 5));
      $con_fechafin_unix = strtotime($convocatoria->con_fechafin_reclamo . ' ' . substr($convocatoria->con_horafin_reclamo, 0, 5));


      if (!($now_unix >= $con_fechainicio_unix
        && $now_unix <= $con_fechafin_unix)) {

        throw new Exception("La etapa de reclamo de la convocatoria ya expiró");
      }

      $fechaActual = new DateTime();
      $fechaFin = new DateTime($convocatoria->con_fechafin_reclamo);
      $interval = $fechaActual->diff($fechaFin);
      $convocatoria->con_diasrestantes = $interval->days;
      $response['success'] = true;
      $response['data']  = compact('convocatoria');
