const AppConvovatoriaWeb = () => {
    const { data, methods, mounted, computed, utilities, el } = {
        ...
        mounted: () => {
            self.postulantId = dom.getAttribute('data-id');
        ...
        methods: {
            ...
            setvalues: (data) => {
                ...
                dom.querySelector('input[name="nombre"]').value = postulante.nombre;
                dom.querySelector('input[name="apellido_paterno"]').value = postulante.apellido_paterno;
                dom.querySelector('input[name="apellido_materno"]').value = postulante.apellido_materno;
                dom.querySelector('input[name="fecha_nacimiento"]').value = postulante.fecha_nacimiento;
                dom.querySelector('input[name="correo"]').value = postulante.correo;
                dom.querySelector('select[name="genero"]').value = postulante.genero;
            ...
            events: () => {
                        ...
                        if (form.checkValidity()) {
                            ...
                            sweet2.show({
                                ...
                                onOk: () => {
                                    ...
                                    $.ajax({
                                        url: window.AppMain.url + `admin/auxiliares/evaluacion/convocatoria/inscripcion/postulante/${self.postulantId}/update`,
                                        method: 'POST',
                                        dataType: 'json',
                                        data: self.formData,
                                        processData: false,
                ...
                self.formInputEvent(false);

            },
            detail: () => {
                return new Promise(function (resolve, reject) {
                    $.ajax({
                        url: window.AppMain.url + `admin/auxiliares/evaluacion/convocatoria/inscripcion/postulante/${self.postulantId}/detail`,
                        method: 'POST',
                        dataType: 'json',
                        cache: 'false'
                    })
            ...
            renderAttachedFileAll: () => {
                const tables = dom.querySelectorAll('.table-attached-file');
                tables.forEach(table => {
                    const tbody = table.querySelector('tbody');
                    self.attachedFileAll.forEach(file => {
                        const tr = document.createElement("tr");
                        tr.innerHTML = `
