const viewfichaDetail = function (pageActive) {
	if (pageActive === void 0) {
	  pageActive = 1;
	}
	...
	function getStatusName(id) {
	  const options = ['NO CUMPLE', 'CUMPLE', 'OBSERVADO'];
	  return options[id];
	}
	...
	function createSelect(options, attributes, events, selected) {
	  if (options === void 0) {
		options = [];
	  }
	  if (attributes === void 0) {
	...
	function setFicha(formData) {
	  return new Promise(function (resolve, reject) {
		$.ajax({
		  url: window.AppMain.url + `admin/auxiliares/evaluaciones/postulaciones/${postulant_id}/ficha`,
		  method: 'POST',
		  dataType: 'json',
		  processData: false,
		  contentType: false,
	...
	function init(data) {
	  ...
	  const isValidFichaRequisito = () => {
		...
		if (brand) {
		  sweet2.show({
			type: 'error',
			text: 'Debe de seleccionar el estado de la ficha'
		  });
		}
		return brand;
	  };
	  const formFichaDetail = () => {
		...
		const headerCells = [createCell('th', 'RUBRO', {
		  class: 'text-center bg-light'
		}, {
		  verticalAlign: 'middle'
		}), createCell('th', 'CRITERIOS', {
		  class: 'text-center bg-light'
		}, {
		  verticalAlign: 'middle'
		}), createCell('th', 'SUBCRITERIOS', {
		  class: 'text-center bg-light'
		}, {
		  verticalAlign: 'middle'
		}), createCell('th', 'EVALUACIÓN', {
		  class: 'text-center bg-light'
		}, {
		  verticalAlign: 'middle'
		})];
		if (self.ficha && self.ficha.promedio == 1) {
		  headerCells.push(createCell('th', 'Puntaje máximo por subcriterio', {
			class: 'text-center bg-light'
		  }, {
			verticalAlign: 'middle'
		  }));
		  headerCells.push(createCell('th', 'Puntaje máximo por rubro', {
			class: 'text-center bg-light'
		  }, {
			verticalAlign: 'middle'
		  }));
		...
		self.sections.forEach((section, index1) => {
		  ...
		  if (section.groups.length == 0) {
			const cells = [createCell('td', section.name, {
			  class: 'colvert bg-light'
			}), createCell('td', ''), createCell('td', ''), createCell('td', '', {
			  class: 'text-center'
			})];
			if (self.ficha && self.ficha.promedio == 1) {
			  cells.push(createCell('td', '', {
				class: 'text-center'
			  }));
			  cells.push(createCell('td', '', {
				class: 'text-center'
			...
			section.groups.forEach((group, index2) => {
			  ...
			  if (group.questions.length == 0) {
				...
				const cells = [createCell('td', div, {
				  class: 'colvert bg-light'
				}, {
				  verticalAlign: 'middle',
				  textAlign: '-webkit-center'
				}), createCell('td', group.name), createCell('td', ''), createCell('td', '', {
				  class: 'text-center'
				})];
				if (self.ficha && self.ficha.promedio == 1) {
				  cells.push(createCell('td', '', {
				...
				group.questions.forEach((question, index3) => {
				  ...
				  if (index3 == 0) {
					cells.push(createCell('td', group.name, {
					  rowspan: lg
					}));
				  }
				  const div = document.createElement('div');
				  ...
				  cells.push(createCell('td', div));
				  cells.push(createCell('td', viewActionOption(question), {
					class: 'text-center'
				  }));
				  if (self.ficha && Number(self.ficha.promedio) == 1) {
					cells.push(createCell('td', question.score, {
					  class: 'text-center'
					}));
					if (index3 == 0 && index2 == 0) {
					  cells.push(createCell('td', section.score, {
						class: 'text-center',
						rowspan: ls
					  }));
					}
		...
		if (self.ficha && Number(self.ficha.promedio) == 1) {
		  rows.push(createRow([createCell('td', 'TOTAL', {
			class: 'text-center colvert bg-light fw-bold',
			colspan: 3
		  }), createCell('td', self.total, {
			class: 'text-center fw-bold',
			id: 'total'
		  }), createCell('td', self.total_question, {
			class: 'text-center fw-bold'
		  }), createCell('td', self.total_section, {
			class: 'text-center fw-bold'
		  })]));
		}
		rows.forEach(row => {
	  ...
	  const saveAll = function (any, page) {
		if (page === void 0) {
		  page = 1;
		}
		if (calculation()) {
	  ...
	  const currentProgress = () => {
		...
		function createTextWrap(circleNumber, labelText, isActive, id) {
		  if (isActive === void 0) {
			isActive = false;
		  }
		  if (id === void 0) {
		...
		function createButton(id, text, isDisabled) {
							  ...
							  <table class="table table-bordered">
								  <thead>
									  <tr>
										  ...
										  ${self.ficha && self.ficha.promedio == 1 ? `<th class="text-center bg-light" style="vertical-align: middle;">Puntaje máximo por subcriterio</th>
		...
		self.sections.forEach((section, index1) => {
		  ...
		  if (section.groups.length == 0) {
			html += `<tr class=""> 
						  ...
						  ${self.ficha && self.ficha.promedio == 1 ? `<td class="text-center"></td>
							   <td class="text-center"></td>` : ``}
					  </tr>`;
		  } else {
			section.groups.forEach((group, index2) => {
			  ...
			  if (group.questions.length == 0) {
				...
				group.questions.forEach((question, index3) => {
				  html += `<tr class="">
									  ...
									  <td>
										  ${question.name}
										  ${question.observation_status == 1 ? `<br><strong>Observacion: </strong> ${question.observation ?? `-`}` : ``}
									  </td>
									  <td class="text-center">${question.value ?? ''}</td>
									  ${self.ficha && self.ficha.promedio == 1 ? `<td class="text-center">${question.score}</td>
				  ...
				  if (question.type == 'selectiva' || question.type == 'marcado' || question.type == 'numerico' || question.type == 'tabla') {
					total = total + value;
				  }
				});
			  }
