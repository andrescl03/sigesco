    ...
    const index = (container) => {
        ...
        const object = {
            data() {
                return {
                    table: {},
                    modalCreateUpdate: new bootstrap.Modal(dom.querySelector('#modalCreateUpdate')),
                    any: 0
                }
            },
            mounted: function () {
            ...
            methods: {
                ...
                clicks: () => {
                    ...
                    forms.forEach(form => {
                        form.addEventListener('submit', (e) => {
                            ...
                            let url = `admin/auxiliares/bonificaciones/store`;
                            if (Number(self.any) > 0) {
                               url = `admin/auxiliares/bonificaciones/${self.any}/update`
                            }
                ...
                pagination: (_callback = ()=>{}) => {
                    self.table = $('#tableIndex').DataTable({
                        ...
                        "dom": '<l<t>ip>',	
                        "ajax": {
                           "url": window.AppMain.url + 'admin/auxiliares/bonificaciones/pagination',
                           "method": "POST",
                           "dataType": "json",
                           "data": {
                           }
                ...
                edit: (id) => {
                    return new Promise((resolve, reject)=>{
                        sweet2.loading();
                        $.ajax({
                            url: window.AppMain.url + `admin/auxiliares/bonificaciones/${id}/edit`,
                            method: 'POST',
                            dataType: 'json',
                            data: {},
                            processData: false,
                ...
                create: () => {
                    return new Promise((resolve, reject)=>{
                        sweet2.loading();
                        $.ajax({
                            url: window.AppMain.url + `admin/auxiliares/bonificaciones/create`,
                            method: 'POST',
                            dataType: 'json',
                            data: {},
                            processData: false,
                ...
                remove: (id) => {
                    return new Promise((resolve, reject)=>{
                        sweet2.loading();
                        $.ajax({
                            url: window.AppMain.url + `admin/auxiliares/bonificaciones/${id}/remove`,
                            method: 'POST',
                            dataType: 'json',
                            data: {},
                            processData: false,
                    ...
                    btnEdits.forEach(btn => {
                        btn.addEventListener('click', async function (e) {
                            try {
                                ...
                                self.any = id;
                                console.log(data);
                                self.setForm(data.bonificacion);
                                self.modalCreateUpdate.show();             
                            } catch (error) {
                                sweet2.show({type:'error', text:error});                             
                            }
                ...
                setForm: (item) => {
                    dom.querySelector('textarea[name="nombre"]').value = item.nombre;
                    dom.querySelector('input[name="puntaje"]').value = item.puntaje;
                }
            },
            renders: {
            },
