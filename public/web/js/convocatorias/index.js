    ...
    $.ajax({
      ...
      .done(function (data) {
        if (!data) {
          ...
          let tableHeader = '<thead><tr><th>Modalidad</th><th>Nivel</th><th>Especialidad</th><th>ACCIONES</th></tr></thead>';
          table.append(tableHeader);
          console.log(data);
          data.data.forEach(function (grupo) {
            let row = '<tr><td>' + grupo.mod_nombre + '</td><td>' + grupo.niv_descripcion + '</td><td>' + grupo.esp_descripcion + '</td><td><a target="_blank" class="btn btn-sm btn-danger" href="' + window.AppMain.url + 'web/convocatorias/' + grupo.con_id + '/inscripciones/' + grupo.grupo_inscripcion_gin_id + '">POSTULAR</a></td></tr>';
            table.append(row);
          });
